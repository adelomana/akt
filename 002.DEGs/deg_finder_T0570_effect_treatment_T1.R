...
metadata = metadata[metadata$time == '06', ]
dim(metadata)
View(metadata)
...
write.table(sorted_filtred_results, file=paste(results_dir, '/effect_treatment_T1_0570.for.tsv', sep=''), quote=FALSE, sep='\t')
write.table(anti_results, file=paste(results_dir, '/effect_treatment_T1_0570.anti.tsv', sep=''), quote=FALSE, sep='\t')

plotPCA(rlog(dds), intgroup=c('genotype')) + ggtitle('effect treatment T1 0570')
...
ggplot() + 
  ...
  labs(x=expression('Expression [log'[2]~'FC]'), y=expression('Significance [log'[10]~'adjusted P]'), color=expression('Expression [log'[10]~'TPM]'), title='effect treatment T1 0570') + 
  theme_linedraw() +
