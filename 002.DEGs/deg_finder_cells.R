...
# drop two samples that in the PCA look like outliers: T84_DMSO_T6_R1 & T84_DMSO_T6_R2
metadata = metadata[!(metadata$labels %in% c('T84_DMSO_T6_R1', 'T84_DMSO_T6_R2')), ]
dim(metadata)
View(metadata)

# drop treated samples, stick to DMSO samples at any time
metadata = metadata[metadata$treatment == 'DMSO', ]
dim(metadata)
View(metadata)
...
dds$genotype = relevel(dds$genotype, ref="T84")
...
a = rowMedians(subset[ , 1:6])
b = rowMedians(subset[ , 7:10])
c = pmax(a, b)
...
sorted_filtred_results$medianTPM_T0570 = rowMedians(subset[rownames(sorted_filtred_results), 1:6]) # T0570
sorted_filtred_results$medianTPM_T84 = rowMedians(subset[rownames(sorted_filtred_results), 7:10]) # T84

write.table(sorted_filtred_results, file=paste(results_dir, '/effect_T0570_vs_T84.for.tsv', sep=''), quote=FALSE, sep='\t')
write.table(anti_results, file=paste(results_dir, '/effect_T0570_vs_T84.anti.tsv', sep=''), quote=FALSE, sep='\t')

plotPCA(rlog(dds), intgroup=c('genotype')) + ggtitle('effect T0570 vs T84')
...
ggplot() + 
  ...
  labs(x=expression('Expression [log'[2]~'FC]'), y=expression('Significance [log'[10]~'adjusted P]'), color=expression('Expression [log'[10]~'TPM]'), title='effect T0570 vs T84') + 
  theme_linedraw() +
