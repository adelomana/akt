...
write.table(sorted_filtred_results, file=paste(results_dir, '/cannonical/effect_treatment_T1_84.for.tsv', sep=''), quote=FALSE, sep='\t')
write.table(anti_results, file=paste(results_dir, '/cannonical/effect_treatment_T1_84.anti.tsv', sep=''), quote=FALSE, sep='\t')
...
ggplot() + 
  ...
  labs(x=expression('Expression [log'[2]~'FC]'), y=expression('Significance [log'[10]~'adjusted P]'), color=expression('Expression [log'[10]~'TPM]'), title='effect treatment T1 84') + 
  theme_linedraw() +
